...
var dlisk = require("../index.js");

describe("DL(i)sk JS", function () {
	...
	it("should have properties", function () {
		...
		properties.forEach(function (property) {
			(dlisk).should.have.property(property);
		});
	});

	describe("crypto.js", function () {
		var crypto = dlisk.crypto;
	...
	describe("dapp.js", function () {
		...
		describe("#createDapp", function () {
			...
			describe("returned dapp", function () {
				var keys = dlisk.crypto.getKeys("secret");
				var secondKeys = dlisk.crypto.getKeys("secret 2");
				...
				it("should be signed correctly", function () {
					var result = dlisk.crypto.verify(trs);
					(result).should.be.ok;
				});
				...
				it("should be second signed correctly", function () {
					trs.amount = 0;
					var result = dlisk.crypto.verifySecondSignature(trs, secondKeys.publicKey);
					(result).should.be.ok;
				});
	...
	describe("delegate.js", function () {
		...
		describe("#createDelegate", function () {
			...
			describe("returned delegate", function () {
				...
				it("should be signed correctly", function () {
					var result = dlisk.crypto.verify(trs, keys.publicKey);
					(result).should.be.ok;
				});
				...
				it("should not be second signed correctly now", function () {
					trs.amount = 100;
					var result = dlisk.crypto.verify(trs, secondKeys.publicKey);
					(result).should.be.not.ok;
				});
	...
	describe("transaction.js", function () {
		var transaction = dlisk.transaction;
		...
		describe("#createTransaction with second secret", function () {
			...
			var keys = dlisk.crypto.getKeys(secondSecret);
			...
			describe("returned transaction", function () {
				...
				it("should be second signed correctly", function () {
					var result = dlisk.crypto.verifySecondSignature(trs, keys.publicKey);
					(result).should.be.ok;
				});
	...
	describe("vote.js", function () {
		...
		describe("#createVote", function () {
			var createVote = vote.createVote,
				vt = null,
				publicKey = dlisk.crypto.getKeys("secret").publicKey,
			...
			describe("returned vote", function () {
				...
				it("should have recipientId string equal to sender", function () {
					(vt).should.have.property("recipientId").and.be.type("string").and.equal(dlisk.crypto.getAddress(publicKey))
				});
				...
				it("should be signed correctly", function () {
					var result = dlisk.crypto.verify(vt);
					(result).should.be.ok;
				});

				it("should be second signed correctly", function () {
					var result = dlisk.crypto.verifySecondSignature(vt, dlisk.crypto.getKeys("second secret").publicKey);
					(result).should.be.ok;
				});
